...
class IonAuthModel
{
	 ...
	 * @author Ben Edmunds
	 */
	public function user(?int $id=null): self
	{
	 ...
	 * @author Ben Edmunds
	 */
	public function getUsersGroups(?int $id=null)
	{
		...
		// If still no id, return empty result
		if (!$id) {
			return $this->db->table($this->tables['users_groups'])->where('1', '0')->get();
		}
	...
	public function inGroup($checkGroup, int $id=0, bool $checkAll=false): bool
	{
		...
		// If no id, user is not in any group
		if (!$id) {
			return false;
		}
	...
	public function addToGroup($groupIds, int $userId=0): int
	{
		...
		// If no userId, cannot add to group
		if (!$userId) {
			return 0;
		}
