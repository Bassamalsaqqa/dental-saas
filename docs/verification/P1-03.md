...
| `session-test/check` | REMOVED |
| `debug/rbac` | REMOVED |
| `debug/role-permissions` | REMOVED |
| `inventory/test-inventory` | REMOVED |
| `inventory/test-inventory-datatable` | REMOVED |

### Routes.php Excerpt (Final state check)
**File:** `app/Config/Routes.php`
```php
// Lines 415-417 (End of File)
    });
});
```
