...
- **Update mechanism:** `updateNotificationCount(count)` updates the badge via:
  ...
  - Reference: `app/Views/layouts/main_auth.php:689-720` (lookup at 690; show/hide + textContent at 714-720)

- **Behavior contract:**
  - When `count > 0`: `textContent = count` and remove `hidden`.
  - When `count === 0`: add `hidden`.


## 3. Unread Count Update Contract

- **Element:** `#notification-count` (badge `<span>`).
  - Reference: `app/Views/layouts/main_auth.php:341`
- **Update mechanism:** `document.getElementById('notification-count')` inside `updateNotificationCount(count)`.
  - Reference: `app/Views/layouts/main_auth.php:689-720` (lookup at 690; update/hide logic at 714-720)
- **Behavior contract:**
  - When `count > 0`: sets `countElement.textContent = count` and removes `hidden`.
